...
class DFlashQwen3Attention(nn.Module):
    ...
    def forward(
        ...
        # Dump post-norm / pre-RoPE.
        try:
            import os
            from pathlib import Path

            dump_dir = Path(
                os.environ.get("MIMO_DFLASH_DUMP_DIR", "/tmp/mimo_dflash_dump")
            )
            enabled = os.environ.get("MIMO_DFLASH_DUMP", "0") in (
                "1",
                "true",
            ) or (dump_dir / "ENABLE").is_file()
            li = int(getattr(self, "layer_idx", -1))

            def _note(suffix, t):
                if not enabled or t is None or not torch.is_tensor(t) or t.numel() == 0:
                    return
                flat = t.detach()
                if flat.ndim >= 2:
                ...
                if flat.shape[0] > 8 and flat.shape[0] % 8 == 0:
                    flat = flat[:8]
                if flat.shape[0] > 8 or li < 0:
                    return
                out_p = dump_dir / f"draft_l{li}_{suffix}.pt"
                if out_p.is_file():
                    return
                try:
                    dump_dir.mkdir(parents=True, exist_ok=True)
                    torch.save(flat.float().cpu().contiguous(), out_p)
                except Exception:
                    pass
        ...
        # Intra-attn dumps for parity (post-RoPE Q/K, V, pre-o_proj).
        try:
            import os
            from pathlib import Path
            ...
            if positions is not None:
                _note("pos", positions.reshape(-1).to(dtype=torch.int64))
            sinks = getattr(self, "attention_sink_bias", None)
            if sinks is not None:
                _note("sinks", sinks.reshape(-1))
        except Exception:
            pass
        ...
        try:
            ...
            li = int(getattr(self, "layer_idx", -1))
            if enabled and li >= 0:
                flat = attn_output.detach().reshape(-1, attn_output.shape[-1])
                if flat.shape[0] > 8 and flat.shape[0] % 8 == 0:
                    flat = flat[:8]
                out_p = dump_dir / f"draft_l{li}_attn_raw.pt"
                if flat.shape[0] <= 8 and not out_p.is_file():
                    dump_dir.mkdir(parents=True, exist_ok=True)
                    torch.save(flat.float().cpu().contiguous(), out_p)
        except Exception:
            pass
...
class DFlashQwen3DecoderLayer(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.self_attn.layer_idx = int(layer_idx)
        self.mlp = Qwen3MLP(
            hidden_size=self.hidden_size,
    ...
    def forward(
        ...
        def _note(suffix, t):
            if not enabled or t is None or not torch.is_tensor(t) or t.numel() == 0:
                return
            flat = t.detach().reshape(-1, t.shape[-1])
            if flat.shape[0] > 8 or li < 0:
                return
            out_p = dump_dir / f"draft_l{li}_{suffix}.pt"
            if out_p.is_file():
        ...
        _note("prenorm", hidden_states)

        hidden_states = self.self_attn(
            positions=positions,
            hidden_states=hidden_states,
        )
        _note("attn", hidden_states)

        hidden_states, residual = self.post_attention_layernorm(hidden_states, residual)
        _note("postnorm", hidden_states)
        hidden_states = self.mlp(hidden_states)
        _note("mlp", hidden_states)
        try:
            _note("out", residual + hidden_states)
        except Exception:
            pass
...
class DFlashQwen3Model(nn.Module):
    ...
    def embed_input_ids(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        try:
            ...
            if os.environ.get("MIMO_DFLASH_DUMP", "0") in ("1", "true") or Path(
                os.environ.get("MIMO_DFLASH_DUMP_DIR", "/tmp/mimo_dflash_dump")
            ).joinpath("ENABLE").is_file():
                if self.mask_token_id is not None:
                    rows = embeds[(input_ids == self.mask_token_id).view(-1)]
                else:
                    rows = embeds
                out = Path(
                    os.environ.get("MIMO_DFLASH_DUMP_DIR", "/tmp/mimo_dflash_dump")
                ...
                flat = embeds.detach().reshape(-1, embeds.shape[-1])
                if flat.shape[0] <= 8 and not (out / "noise_embed_full.pt").is_file():
                    torch.save(flat.float().cpu(), out / "noise_embed_full.pt")
                if not (out / "noise_embed.pt").is_file():
                    torch.save(rows.detach().float().cpu(), out / "noise_embed.pt")
                if (
                    flat.shape[0] <= 8
                    and not (out / "draft_input_ids.pt").is_file()
                ):
                    torch.save(
                        input_ids.detach().reshape(-1).to(torch.int64).cpu(),
                        out / "draft_input_ids.pt",
                    )
        except Exception:
            pass
        return embeds
    ...
    def precompute_and_store_context_kv(
        ...
        try:
            ...
            if enabled:
                dump_dir.mkdir(parents=True, exist_ok=True)
                # context_states here is fc(aux) pre-hidden_norm (combine_hidden_states).
                if not (dump_dir / "ctx_hidden_prenorm.pt").is_file():
                    torch.save(
                        context_states.detach().float().cpu().contiguous(),
                        dump_dir / "ctx_hidden_prenorm.pt",
                    )
                if not (dump_dir / "ctx_hidden.pt").is_file():
                    torch.save(
                        normed_context_states.detach().float().cpu().contiguous(),
                        dump_dir / "ctx_hidden.pt",
                    )
                if not (dump_dir / "ctx_positions.pt").is_file():
                    torch.save(
                        context_positions.detach().cpu().contiguous(),
                        dump_dir / "ctx_positions.pt",
                    )
                for li in range(min(L, 5)):
                    kp = dump_dir / f"ctx_l{li}_k.pt"
                    vp = dump_dir / f"ctx_l{li}_v.pt"
                    if not kp.is_file():
                        torch.save(
                            all_k_final[li]
                            .detach()
    ...
    def forward(
        ...
        for li, layer in enumerate(self.layers):
            ...
            try:
                ...
                if enabled and not (dump_dir / f"draft_h_l{li}.pt").is_file():
                    if residual is None:
                        complete = hidden_states
                    else:
                        complete = residual + hidden_states
                    flat = complete.detach().reshape(-1, complete.shape[-1])
                    # Prefer single block (B<=8); skip batched.
                    if flat.shape[0] <= 8:
                        dump_dir.mkdir(parents=True, exist_ok=True)
                        torch.save(
                            flat.float().cpu(), dump_dir / f"draft_h_l{li}.pt"
                        )
            except Exception:
                pass
        ...
        try:
            ...
            if enabled and not (dump_dir / "draft_hidden.pt").is_file():
                flat = hidden_states.detach().reshape(-1, hidden_states.shape[-1])
                if flat.shape[0] <= 8:
                    dump_dir.mkdir(parents=True, exist_ok=True)
                    torch.save(flat.float().cpu(), dump_dir / "draft_hidden.pt")
        except Exception:
            pass
...
class DFlashQwen3ForCausalLM(Qwen3ForCausalLM):
    ...
    def compute_logits(
        ...
        try:
            ...
            if enabled and out is not None and not (dump_dir / "draft_logits.pt").is_file():
                dump_dir.mkdir(parents=True, exist_ok=True)
                torch.save(out.detach().float().cpu(), dump_dir / "draft_logits.pt")
                import json
                import time
                    ...
                    json.dumps(
                        {
                            ...
                            "ts": time.time(),
                            "shapes": {
                                "draft_logits": list(out.shape),
                            },
                        },
                        indent=2,
                    )
                )
                enable = dump_dir / "ENABLE"
                if enable.is_file():
                    enable.unlink()
        except Exception:
            pass
    ...
    def combine_hidden_states(
        ...
        try:
            ...
            if enabled:
                dump_dir.mkdir(parents=True, exist_ok=True)
                # First dump wins (prefill/decode first combine).
                if not (dump_dir / "aux.pt").is_file():
                    torch.save(
                        hidden_states.detach().float().cpu(), dump_dir / "aux.pt"
                    )
                if not (dump_dir / "projected.pt").is_file():
                    torch.save(
                        result.detach().float().cpu(), dump_dir / "projected.pt"
                    )
        except Exception:
            pass
        return result
