...
if (
    'layer.register_parameter("weight_scale_inv", weight_scale)' in text
    and "mxfp8_config: ModelOptMxFp8Config" in text
    and "self.mxfp8_config = mxfp8_config" in text
    and 'quant_algo == "MXFP8"' in text
    and "ModelOptMxFp8LinearMethod(self.mxfp8_config)" in text
    and "ModelOptMxFp8FusedMoE(" in text
):
    print('[fix-modelopt-mixed-mxfp8] ModelOpt mixed MXFP8 support already present')
    raise SystemExit
...
if new not in text:
    if old not in text:
        if 'layer.register_parameter("weight_scale_inv", weight_scale)' in text:
